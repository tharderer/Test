...
def run_blender(repo_root: Path, script: Path, args: list[str]) -> subprocess.CompletedProcess[str]:
    cmd = [
        str(blender_binary(repo_root)), '--background', '--factory-startup', '--python-exit-code', '2', '--python',
        str(script), '--', *args,
    ]
    return subprocess.run(cmd, cwd=repo_root, check=True, text=True, timeout=900)
