...
def _surface_ratios(body, gear, depsgraph, max_distance: float, *, ignore_below_z: float | None = None) -> tuple[float, float]:
    ...
    if bvh is None:
        raise RuntimeError('could not build Abraham body BVH')
    ...
    for world_pos, world_normal in _evaluated_positions_normals(gear, depsgraph):
        if ignore_below_z is not None and world_pos.z < ignore_below_z:
            continue
        eligible += 1
        local_pos = body_inv @ world_pos
        nearest, _normal, _index, distance = bvh.find_nearest(local_pos)
        if nearest is None:
            floating += 1
            continue
        if distance > max_distance:
            floating += 1
        if distance < 0.004:
            nearest_world = body.matrix_world @ nearest
            direction = nearest_world - world_pos
            if direction.length > 1e-8 and world_normal.dot(direction.normalized()) < 0.0:
                penetrating += 1
    if eligible == 0:
        return 0.0, 0.0
...
def _staff_socket_drift(armature, staff) -> float:
    ...
    if socket is None:
        return float('inf')
    local_offset = Vector((0.0, -0.035, 0.0))
